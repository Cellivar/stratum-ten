...
#endif    
    //
    // to check timing between sending command and transmit complete for example,
    // modify a hardware bit here so we can watch on oscilloscope.
    //
    // PORTH &= ~0x20; // turn off debug flag indicating that command has been sent to W5500
    //
    return 1;
}
