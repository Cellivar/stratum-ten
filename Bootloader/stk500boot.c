...
//************************************************************************
//* March, 2016     Added code to pass the original value of the MCU's SR to app upon exit of bootloader.
//*                 This would allow the app to determine the reason for reset -- power fail/WDT/etc...
//*                 This code is not currently enabled and is not tested.
//************************************************************************
...
#ifdef PASS_MCUSR_TO_APP
//
// this code was copied from optiboot -- it has not been tested with this
// bootloader. the original bootloader code is used if the macro is not defined.
//
...
#ifdef PASS_MCUSR_TO_APP
    if (mcuStatusReg & (_BV(WDRF) | _BV(BORF) | _BV(PORF))
    {
